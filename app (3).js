...
function filtrosHTML(opts) {
  ...
  if (opts.estado) {
    html += `<div class="fg"><label>Estado compromiso</label><select id="fEstado" onchange="setFiltro('estado',this.value)">
      <option value="__TODOS__">Todos</option>
      <option value="PENDIENTE" ${curFiltroEstado==='PENDIENTE'?'selected':''}>Pendientes</option>
...
function renderResumen() {
  ...
  const compPendientes = comps.length - compConcluidos;
  ...
  return `
    ...
    <div class="kpis">
      ...
      ${kpiCard("Compromisos pendientes", compPendientes, `de ${comps.length} registrados`, "a", "⏳")}
    ...
    <div class="card"><h3>Tendencia semanal <span class="cnt">últimos 7 días vs anteriores</span></h3>
      <div class="trend-row-wrap">
        ...
        ${tendenciaCard("Compromisos resueltos", compConcluidos, comps.length - compConcluidos, `${compPendientes} pendientes`)}
      </div>
    </div>
...
function renderCompromisosPage() {
  const comps = DATA.compromisos.filter(c => {
    ...
    if (curFiltroEstado !== "__TODOS__") {
      ...
      if (curFiltroEstado === "PENDIENTE" && (esConcluido || esProceso)) return false;
    }
    if (curBusqueda) {
      const q = curBusqueda.toLowerCase();
  ...
  const pendientes = comps.length - concluidos - enProceso;
  ...
  const rows = comps.slice(0, 150)
    ...
    let diasHtml = '<span class="pill g">— sin fecha</span>';
    if (c.diasActivo !== null) {
      if (c.cerrado) {
        diasHtml = `<span class="pill verde" style="font-size:12px;padding:4px 12px">✔️ ${c.diasActivo} días</span>`;
      } else if (c.vencido) {
        diasHtml = `<span class="pill rojo" style="font-size:12px;padding:4px 12px;font-weight:900">⚠️ ${c.diasActivo} días</span>`;
      } else {
        diasHtml = `<span class="pill ambar" style="font-size:12px;padding:4px 12px">⏳ ${c.diasActivo} días</span>`;
      }
    }
  ...
  return `
    ...
    <div class="kpis">
      ...
      ${kpiCard("Pendientes", pendientes, pct(pendientes, comps.length) + "% del total", "", "⏳")}
    </div>
    <div class="kpis">
      ${kpiCard("Vencidos", vencidos, "pendientes y pasados de la fecha meta", "", "🚨")}
      ...
      <div class="card"><h3>Tasa de resolución</h3>
        <div class="gauge-wrap">${gaugeComp}
          <div style="flex:1;min-width:160px">
            ...
            <div class="miniStat"><span class="msDot" style="background:#C8102E"></span><span class="msLbl">Pendientes</span><span class="msVal">${pendientes}</span></div>
    ...
    <div class="card"><h3>Detalle de compromisos <span class="cnt">${comps.length} resultados</span></h3>
      <div class="insight b"><b>⏱️ Contador de días:</b> para compromisos <b>concluidos</b> (✔️ verde), muestra los días entre emisión y cierre real. Para compromisos <b>pendientes</b> (⏳ ámbar), muestra los días transcurridos desde la emisión hasta hoy. Si un pendiente ya pasó su fecha meta, se marca <b>vencido</b> (⚠️ rojo).</div>
      <table style="margin-top:14px"><thead><tr><th>Shipper</th><th>Tipo</th><th>Ejecutivo</th><th class="ce">Emisión</th><th class="ce">Cierre / Meta</th><th class="ce">Días transcurridos</th><th class="ce">Estado</th><th>Observaciones</th></tr></thead>
