...
// Cada pestaña conserva sus propios filtros para evitar que una selección
// en una vista contamine las demás.
const FILTROS_PAGINA = {};
function filtrosPagina(page = curPage) {
  if (!FILTROS_PAGINA[page]) FILTROS_PAGINA[page] = { mes: "__TODOS__", vend: "__TODOS__", tipo: "__TODOS__", estado: "__TODOS__", visita: "__TODOS__", compTipo: "__TODOS__", busq: "" };
  return FILTROS_PAGINA[page];
}
function setFiltroPagina(page, tipo, val) { filtrosPagina(page)[tipo] = val; }
...
// FILTRO BASE
// ============================================================
function actasFiltradas(page = curPage) {
  const f = filtrosPagina(page);
  return DATA.actas.filter(a => {
    if (f.mes !== "__TODOS__" && a.mes !== f.mes) return false;
    if (f.vend !== "__TODOS__" && a.responsable !== f.vend) return false;
    if (f.tipo !== "__TODOS__" && a.tipo !== f.tipo) return false;
    if (f.busq) {
...
function filtrosHTML(opts) {
  opts = opts || {};
  const f = filtrosPagina(curPage);
  const meses = mesesOrdenados(DATA.meses);
  const vends = DATA.vendedores;
  let html = '<div class="filters">';
  if (opts.mes !== false) {
    html += `<div class="fg"><label>Mes</label><select onchange="setFiltro('mes',this.value)">
      <option value="__TODOS__">Todos los meses</option>
      ${meses.map(m => `<option value="${m}" ${f.mes === m ? "selected" : ""}>${m.charAt(0) + m.slice(1).toLowerCase()}</option>`).join("")}
    </select></div>`;
  }
  if (opts.vend !== false) {
    html += `<div class="fg"><label>Vendedor</label><select onchange="setFiltro('vend',this.value)">
      <option value="__TODOS__">Todos</option>
      ${vends.map(v => `<option value="${v}" ${f.vend === v ? "selected" : ""}>${v}</option>`).join("")}
    </select></div>`;
  }
  if (opts.tipo !== false) {
    html += `<div class="fg"><label>Tipo de acta</label>
      <div class="chipbar">
        <button class="chip ${f.tipo === '__TODOS__' ? 'active' : ''}" onclick="setFiltro('tipo','__TODOS__')">Todas</button>
        <button class="chip mant ${f.tipo === 'mant' ? 'active' : ''}" onclick="setFiltro('tipo','mant')">🔧 Mantenimiento</button>
        <button class="chip nuevo ${f.tipo === 'nuevo' ? 'active' : ''}" onclick="setFiltro('tipo','nuevo')">🆕 Cliente nuevo</button>
        <button class="chip varios ${f.tipo === 'varios' ? 'active' : ''}" onclick="setFiltro('tipo','varios')">📌 Asuntos varios</button>
        <button class="chip ${f.tipo === 'otro' ? 'active' : ''}" onclick="setFiltro('tipo','otro')" style="${f.tipo==='otro'?'background:#F59E0B':''}">⚠️ Sin clasificar</button>
      </div></div>`;
  }
  if (opts.estado) {
    html += `<div class="fg"><label>Estado compromiso</label><select onchange="setFiltro('estado',this.value)">
      <option value="__TODOS__">Todos</option>
      <option value="ABIERTO" ${f.estado==='ABIERTO'?'selected':''}>Abiertos</option>
      <option value="PROCESO" ${f.estado==='PROCESO'?'selected':''}>En proceso</option>
      <option value="CONCLUIDO" ${f.estado==='CONCLUIDO'?'selected':''}>Concluidos</option>
    </select></div>`;
  }
  if (opts.compTipo) {
    const tipos = [...new Set(DATA.compromisos.map(c => c.tipo).filter(Boolean))].sort();
    html += `<div class="fg"><label>Tipo de compromiso</label><select onchange="setFiltro('compTipo',this.value)">
      <option value="__TODOS__">Todos</option>
      ${tipos.map(t => `<option value="${esc(t)}" ${f.compTipo===t?'selected':''}>${esc(t)}</option>`).join("")}
    </select></div>`;
  }
  if (opts.buscar !== false) {
    html += `<div class="fg grow searchbox"><label>Buscar</label><input type="text" placeholder="Cliente, código, texto…" value="${esc(f.busq)}" oninput="setFiltro('busq',this.value)"></div>`;
  }
  html += "</div>";
  return html;
}
...
function setFiltro(tipo, val) {
  const f = filtrosPagina(curPage);
  f[tipo] = val;
  // Compatibilidad con código existente que todavía consulte las variables antiguas.
...
function obligatoriasFiltradas() {
  const f = filtrosPagina("obligatorias");
  const mesActual = mesesOrdenados(DATA.meses).slice(-1)[0];
  return DATA.asignacion.filter(a => {
    if (!a.obligatoria) return false;
    if (f.vend !== "__TODOS__" && a.ejecutivo !== f.vend) return false;
    const vis = a.visitas[mesActual] || "—";
    if (f.visita !== "__TODOS__" && vis !== f.visita) return false;
    if (f.busq) {
      const q = f.busq.toLowerCase();
...
function filtrosObligatoriasHTML() {
  ...
  return `<div class="filters">
    ...
    <div class="fg"><label>Estado de visita</label>
      <div class="chipbar">
        <button class="chip ${f.visita === '__TODOS__' ? 'active' : ''}" onclick="setFiltro('visita','__TODOS__')">Todas</button>
        <button class="chip ${f.visita === 'VISITADO' ? 'active' : ''}" onclick="setFiltro('visita','VISITADO')" style="${f.visita==='VISITADO'?'background:#12B76A':''}">✅ Visitado</button>
        <button class="chip ${f.visita === 'PENDIENTE' ? 'active' : ''}" onclick="setFiltro('visita','PENDIENTE')" style="${f.visita==='PENDIENTE'?'background:#F59E0B':''}">⏳ Pendiente</button>
      </div></div>
...
function renderObligatoriasPage() {
  ...
  return `
    ...
    <div class="card"><h3>Listado de cuentas con visita obligatoria <span class="cnt">${lista.length} de ${todasOblig.length}</span></h3>
      ...
      <tbody>${rows || '<tr><td colspan="10" class="empty">Sin resultados con estos filtros</td></tr>'}</tbody></table>
    </div>
  `;
}
...
function renderVendedoresPage() {
  const actas = actasFiltradas();
  const porVend = {};
  DATA.vendedores.forEach(v => porVend[v] = { total: 0, mant: 0, nuevo: 0, varios: 0, otro: 0, clientes: new Set(), compTotal: 0, compCerrados: 0, compAbiertos: 0 });
  actas.forEach(a => {
    if (!porVend[a.responsable]) porVend[a.responsable] = { total: 0, mant: 0, nuevo: 0, varios: 0, otro: 0, clientes: new Set(), compTotal: 0, compCerrados: 0, compAbiertos: 0 };
  ...
  DATA.compromisos.forEach(c => {
    if (!porVend[c.ejecutivo]) porVend[c.ejecutivo] = { total: 0, mant: 0, nuevo: 0, varios: 0, otro: 0, clientes: new Set(), compTotal: 0, compCerrados: 0, compAbiertos: 0 };
    const p = porVend[c.ejecutivo];
    p.compTotal++;
    if (c.estado.includes("CONCLUIDO") || c.estado.includes("CERRADO")) p.compCerrados++;
    else p.compAbiertos++;
  });

  const activos = Object.entries(porVend).filter(([v, d]) => d.total > 0 || d.compTotal > 0);
  const filas = activos.sort((a, b) => b[1].total - a[1].total)
    .map(([v, d]) => `<tr class="rowclick" onclick="setFiltro('vend','${esc(v)}');nav('actas',Array.from(document.querySelectorAll('.tab')).find(t=>t.textContent.includes('Actas')))">
      ...
      <td class="ce">${d.compTotal}</td><td class="ce">${d.compCerrados} (${pct(d.compCerrados,d.compTotal)}%)</td><td class="ce">${d.compAbiertos} (${pct(d.compAbiertos,d.compTotal)}%)</td>
    </tr>`).join("");
  ...
  return `
    ...
    <div class="card"><h3>Detalle por vendedor <span class="cnt">${activos.length} activos</span></h3>
      <table><thead><tr><th>Vendedor</th><th class="ce">Total actas</th><th class="ce">Mantenim.</th><th class="ce">Cliente nuevo</th><th class="ce">Asuntos varios</th><th class="ce">Clientes únicos</th><th class="ce">% Mantenim.</th><th class="ce">Compromisos</th><th class="ce">Cerrados</th><th class="ce">Abiertos</th></tr></thead>
      <tbody>${filas || '<tr><td colspan="10" class="empty">Sin datos</td></tr>'}</tbody></table>
...
function renderCompromisosPage() {
  const f = filtrosPagina("compromisos");
  const comps = DATA.compromisos.filter(c => {
    if (f.mes !== "__TODOS__" && c.mes !== f.mes) return false;
    if (f.vend !== "__TODOS__" && c.ejecutivo !== f.vend) return false;
    if (f.compTipo !== "__TODOS__" && c.tipo !== f.compTipo) return false;
    if (f.estado !== "__TODOS__") {
      const esConcluido = c.estado.includes("CONCLUIDO") || c.estado.includes("CERRADO");
      const esProceso = c.estado.includes("PROCESO");
      if (f.estado === "CONCLUIDO" && !esConcluido) return false;
      if (f.estado === "PROCESO" && !esProceso) return false;
      if (f.estado === "ABIERTO" && (esConcluido || esProceso)) return false;
    }
    if (f.busq) {
      const q = f.busq.toLowerCase();
      if (!(c.shipper.toLowerCase().includes(q) || c.tipo.toLowerCase().includes(q) || c.observaciones.toLowerCase().includes(q) || c.ejecutivo.toLowerCase().includes(q))) return false;
    }
    return true;
  });
  ...
  const porVendComp = {};
  comps.forEach(c => {
    if (!porVendComp[c.ejecutivo]) porVendComp[c.ejecutivo] = { total: 0, cerrados: 0, proceso: 0, abiertos: 0 };
    const p = porVendComp[c.ejecutivo];
    p.total++;
    if (c.estado.includes("CONCLUIDO") || c.estado.includes("CERRADO")) p.cerrados++;
    else if (c.estado.includes("PROCESO")) p.proceso++;
    else p.abiertos++;
  });
  const vendCompRows = Object.entries(porVendComp).sort((a,b) => b[1].total - a[1].total).map(([v,d]) =>
    `<tr><td class="nm">${esc(v)}</td><td class="ce">${d.total}</td><td class="ce">${d.cerrados} (${pct(d.cerrados,d.total)}%)</td><td class="ce">${d.proceso} (${pct(d.proceso,d.total)}%)</td><td class="ce">${d.abiertos} (${pct(d.abiertos,d.total)}%)</td></tr>`
  ).join("");
  ...
  return `
    ${filtrosHTML({ tipo: false, estado: true, compTipo: true })}
    <div class="kpis">
    ...
    <div class="card"><h3>Clasificación de compromisos por vendedor <span class="cnt">${Object.keys(porVendComp).length} vendedores</span></h3>
      <table><thead><tr><th>Vendedor</th><th class="ce">Total</th><th class="ce">Cerrados</th><th class="ce">En proceso</th><th class="ce">Abiertos</th></tr></thead>
      <tbody>${vendCompRows || '<tr><td colspan="5" class="empty">Sin datos</td></tr>'}</tbody></table>
    </div>
